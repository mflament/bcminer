...
import static com.infine.demo.bcminer.BlockHeader.HEADER_INTS;
...
public class Sha256 {
    ...
    public static int[] createMidstate(BlockHeader header) {
        ...
        createMidstate(midstate, header);
        return midstate;
    }

    public static void createMidstate(int[] midstate, BlockHeader header) {
    ...
    public static void updateHash(int[] hash, int[] data, int[] midstate, int[] workBuffer, int nonce) {
        ...
        workBuffer[1] = data[BlockHeader.TIME]; // time
        workBuffer[2] = data[BlockHeader.NBITS]; // nbits
